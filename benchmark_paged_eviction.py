#!/usr/bin/env python3
"""
PagedEviction vs Standard vLLM Benchmark
Run in Google Colab to compare performance
"""

# ============================================
# CELL 1: Install vLLM (run this first)
# ============================================
import subprocess
import sys
...
# Install vLLM from your fork
subprocess.check_call([sys.executable, "-m", "pip", "install", "transformers", "--quiet"])

print("\n✓ Base packages installed!")
print("\nNow installing vLLM from your fork...")

try:
    subprocess.check_call([sys.executable, "-m", "pip", "install", "vllm", "--quiet"])
    print("\n✓ vLLM installed from PyPI!")
except:
    print("\n⚠️  PyPI install failed, trying from your fork...")
    subprocess.check_call(["git", "clone", "https://github.com/PulkitChatwal/vllm.git"])
    subprocess.check_call([sys.executable, "-m", "pip", "install", "-e", "vllm", "--quiet"])
    print("\n✓ vLLM installed from your fork!")
...
# ============================================
# CELL 3: Define test prompts (long contexts)
# ============================================
# These are long prompts to stress-test the KV cache
...
def benchmark_model(llm, prompts, max_tokens=256, name="Model"):
    ...
    tokens_per_second_per_prompt = throughput / len(prompts)
    ...
    metrics = {
        ...
        "max_tokens": max_tokens,
        "tokens_per_prompt": total_tokens / len(prompts),
    }

    return metrics, outputs

# ============================================
# CELL 5: Run benchmark - Standard vLLM
# ============================================
print("=" * 60)
...
# ============================================
# CELL 6: Run benchmark - PagedEviction
# ============================================
print("\n" + "=" * 60)
print("BENCHMARK 2: vLLM with PagedEviction")
print("=" * 60)

# Load vLLM with PagedEviction
llm_paged = LLM(
    model="meta-llama/Llama-3.2-1B-Instruct",
    gpu_memory_utilization=0.9,
    max_model_len=8192,
    enable_paged_eviction=True,  # ← PagedEviction enabled!
    paged_eviction_budget=1024,  # ← Keep only 1024 tokens in cache
...
# ============================================
# CELL 8: Test with different budgets
# ============================================
print("\n" + "=" * 60)
...
for budget in budgets:
    ...
    metrics, _ = benchmark_model(
        llm,
        test_prompts[:3],  # Use 3 prompts for faster testing
        max_tokens=256,
...
print("=" * 60)
print(f"\n🎉 PagedEviction achieved:")
print(f"  • {throughput_speedup:.2f}x throughput improvement")
print(f"  • {time_speedup:.2f}x time speedup")
print(f"  • {memory_saved_pct:.1f}% memory savings")
print("\n" + "=" * 60)

# ============================================
# CELL 10: Visualize results (optional)
# ============================================
try:
    ...
    plt.show()
    print("✓ Budget plot saved as budget_comparison.png")

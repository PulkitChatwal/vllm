#!/usr/bin/env python3
"""
Colab-Compatible PagedEviction Benchmark
Handles CUDA compatibility issues
"""

# ============================================
# STEP 1: Install compatible vLLM version
# ============================================
import subprocess
import sys

print("Installing compatible vLLM for Colab...")

# Uninstall existing vLLM
subprocess.run([sys.executable, "-m", "pip", "uninstall", "-y", "vllm"], capture_output=True)

# Install vLLM version compatible with Colab's CUDA
# Colab typically has CUDA 12.x, so we need vLLM that works with that
subprocess.check_call([
    sys.executable, "-m", "pip", "install",
    "vllm==0.6.3.post1",  # Stable version with CUDA 12 support
    "--quiet"
])
...
print(f"\n⏱️  Time:")
print(f"  Standard: {standard_metrics['total_time']:.2f}s")
print(f"  PagedEviction: {paged_metrics['total_time']:.2f}s")

print(f"\n🚀 Throughput:")
print(f"  Standard: {standard_metrics['throughput_tokens_per_sec']:.2f} tok/s")
print(f"  PagedEviction: {paged_metrics['throughput_tokens_per_sec']:.2f} tok/s")

print(f"\n💾 Memory:")
print(f"  Standard: {standard_metrics['peak_memory_gb']:.2f} GB")
